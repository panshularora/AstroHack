...
export function NotificationCenter() {
  ...
  const getIcon = (category: string) => {
    switch (category) {
      case 'Predictions': return <Target className="w-4 h-4 text-brand" />
        ...
        {tabs.map(tab => (
          <button 
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${
              activeTab === tab ? 'bg-white text-black' : 'bg-white/5 text-[#9CA3AF] hover:bg-surface-3 hover:text-white'
            }`}
          >
            {tab}
          </button>
