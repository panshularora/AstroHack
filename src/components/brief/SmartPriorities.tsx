...
export function SmartPriorities() {
  const getIcon = (type: string) => {
    switch(type) {
      ...
      case 'prediction': return <Target className="w-5 h-5 text-primary" />
  ...
  const getGradient = (type: string) => {
    switch(type) {
      ...
      case 'prediction': return 'from-primary/10 to-transparent border-primary/20'
    ...
    <div className="mb-16">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {mockSmartPriorities.map((priority) => (
          <div key={priority.id} className={`bg-gradient-to-br ${getGradient(priority.type)} bg-card border rounded-3xl p-6 flex flex-col hover:-translate-y-1 transition-transform duration-300`}>
            <div className="w-10 h-10 rounded-2xl bg-black/40 border border-white/5 flex items-center justify-center mb-4 shadow-inner">
            ...
            <Button variant="outline" className="w-full h-10 text-xs font-bold border-white/10 hover:bg-white/5">
              {priority.actionText}
            </Button>
          </div>
        ))}
