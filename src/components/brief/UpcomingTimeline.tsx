...
export function UpcomingTimeline() {
  const events = [
    ...
    { day: "Aug 15", time: "Prediction", title: "Tech Job Offer Window Opens", type: "prediction", icon: <Clock className="w-4 h-4 text-primary" /> },
      ...
      <div className="relative pl-6 border-l-2 border-white/10 space-y-8">
        {events.map((event, i) => (
          <div key={i} className="relative group cursor-pointer">
            <div className="absolute -left-[35px] top-0 w-8 h-8 rounded-full bg-[#1a1b26] border border-white/10 flex items-center justify-center group-hover:border-primary/50 transition-colors">
              {event.icon}
            </div>
