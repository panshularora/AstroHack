...
export function ContextPanel() {
  ...
  return (
    <div className="bg-surface border border-line rounded-lg p-6 md:p-8 relative overflow-hidden h-full">
      <div className="absolute top-0 right-0 p-32 bg-brand/5 rounded-full blur-3xl -z-10" />
      ...
      <div className="space-y-5">
        {items.map(({ key, label, desc }) => (
          <div key={key} className="flex items-center justify-between group cursor-pointer" onClick={() => toggleState(key)}>
            ...
            {toggles[key] ? (
              <ToggleRight className="w-6 h-6 text-brand transition-colors" />
            ) : (
