...
export function MilestoneNode({ milestone, index }: { milestone: Milestone, index: number }) {
  ...
  return (
    <div className={`relative flex flex-col md:flex-row items-start gap-8 w-full group ${isEven ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
      ...
      <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 w-8 h-8 rounded-full border-4 border-background bg-surface items-center justify-center shadow-[0_0_15px_rgba(139,92,246,0.3)] z-10 transition-transform group-hover:scale-125 top-8">
        <div className="w-2 h-2 rounded-full bg-brand" />
      </div>

      {/* Node Dot (Visible on Mobile) */}
      <div className="md:hidden absolute left-0 w-6 h-6 -translate-x-[11px] rounded-full border-4 border-background bg-surface flex items-center justify-center shadow-[0_0_15px_rgba(139,92,246,0.3)] z-10 top-8">
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center justify-between text-xs text-[#9CA3AF] border-t border-line-subtle pt-4">
        ...
        <AnimatePresence>
          {expanded && (
            <motion.div
              ...
              className="relative z-10 mt-4 space-y-3 pt-4 border-t border-line-subtle"
            >
