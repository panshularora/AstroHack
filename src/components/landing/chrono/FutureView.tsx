...
import type { TimeCapsule, FutureGoal } from '../../../types/chrono';
      ...
      <div className="p-5 rounded-2xl bg-neutral-900/60 border border-neutral-800 flex flex-col space-y-4 shadow-lg">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-white font-bold text-xs font-mono">
            ...
            <span>Future Horizons & Intentions</span>
          </div>
          <span className="text-[10px] font-mono text-neutral-500">Upcoming Timeblocks</span>
        </div>

        <form onSubmit={handleAddGoal} className="flex flex-col sm:flex-row items-center gap-2 font-mono text-xs">
          <input
            type="text"
            value={goalText}
          ...
          <div className="space-y-2 pt-1 font-mono text-xs">
            {futureGoals.map((goal) => (
              ...
              >
                ...
                <span className="text-[10px] text-neutral-500">{goal.timeLabel}</span>
              </div>
            ))}
          </div>
        ) : (
          <div className="text-xs text-neutral-500 text-center py-2 font-mono">
            No future goals set yet.
          </div>
        )}
      </div>
      ...
      <div className="p-5 rounded-2xl bg-neutral-900/60 border border-neutral-800 flex flex-col space-y-4 shadow-lg">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-mono text-neutral-300 hover:text-white bg-black/40 hover:bg-neutral-800 px-3 py-1.5 rounded-full border border-neutral-800 transition-all cursor-pointer"
          >
                ...
                >
                  ...
                  {isUnlocked ? (
                    ...
                    <div className="text-[10px] text-neutral-500 italic">
                      Sealed message locked until target timestamp...
                    </div>
                  )}
                </div>
              );
