  ...
  return (
    <div className="mb-16">
      ...
      <div className="relative pl-8 md:pl-10 space-y-10">
        {/* Continuous Line */}
        <div className="absolute left-[11px] md:left-[19px] top-4 bottom-0 w-px bg-gradient-to-b from-brand/50 via-white/10 to-transparent" />
        ...
        {mockTimelineEvents.map((event, i) => {
          ...
          if (event.type === "milestone") {
            return (
              ...
              >
                ...
                <div className="bg-gold/5 border border-gold/10 rounded-lg p-5 md:p-6 flex items-center justify-between ml-2">
                  <div>
          ...
          return (
            ...
            >
              <div className="absolute left-[-2rem] md:left-[-2.5rem] w-6 h-6 bg-brand/20 rounded-full flex items-center justify-center border border-brand/30 group-hover:bg-brand/40 transition-colors shadow-[0_0_10px_rgba(124,58,237,0.2)]">
                <CalendarCheck className="w-3 h-3 text-brand" />
              </div>
              
              <div 
                className={`bg-surface border ${isExpanded ? 'border-brand/50 shadow-[0_0_20px_rgba(124,58,237,0.1)]' : 'border-line/60 hover:border-line-strong'} rounded-lg p-6 md:p-8 ml-2 transition-all cursor-pointer`}
                onClick={() => setExpandedId(isExpanded ? null : event.id)}
              >
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                  <div className="flex items-start gap-4">
                    <img src={event.astrologer?.avatar} alt={event.astrologer?.name} className="w-12 h-12 rounded-full border-2 border-line/60 object-cover shrink-0" />
                    <div>
                      ...
                      <p className="text-sm text-brand font-medium">with {event.astrologer?.name}</p>
                    </div>
                  </div>
                  ...
                  {isExpanded && (
                    ...
                    >
                      <div className="pt-6 grid grid-cols-3 gap-3 md:gap-4 mb-6">
                        <div className="bg-white/5 rounded-lg p-4 text-center border border-line-subtle">
                        ...
                        <div className="bg-white/5 rounded-lg p-4 text-center border border-line-subtle">
                          <Moon className="w-5 h-5 text-ink-secondary mx-auto mb-2" />
                        ...
                        <div className="bg-white/5 rounded-lg p-4 text-center border border-line-subtle">
                          <MessageSquare className="w-5 h-5 text-brand mx-auto mb-2" />
                      ...
                      <div className="flex gap-3">
                        ...
                        <Button variant="outline" className="flex-1 h-12 text-sm font-bold border-line-strong text-white hover:bg-white/5">Replay Audio</Button>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
