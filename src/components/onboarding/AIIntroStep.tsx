...
export function AIIntroStep({ onNext, data }: { onNext: () => void, data: OnboardingData }) {
  ...
  return (
    <div className="flex-1 flex flex-col p-6 max-w-3xl mx-auto w-full relative min-h-screen pt-12 md:pt-24">
      ...
      <div className="bg-card border border-white/10 rounded-[32px] p-6 relative overflow-hidden flex-1 max-h-[500px] flex flex-col shadow-2xl">
          ...
          <AnimatePresence>
            ...
            {messages.map((msg, i) => (
              ...
              >
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl rounded-tl-none p-4 text-white text-sm leading-relaxed">
                  {msg.text}
                </div>
              </motion.div>
            ))}
        ...
        <AnimatePresence>
          {showNext && (
            ...
            >
              <button className="px-4 py-2 bg-white/5 border border-white/10 rounded-full text-xs text-white hover:bg-white/10 transition-colors">
        ...
        <div className="mt-4 relative">
          <input 
            ...
            disabled 
            className="w-full bg-black/40 border border-white/10 rounded-full pl-4 pr-12 py-3 text-sm text-[#9CA3AF] cursor-not-allowed"
          />
