...
export function FeatureShowcase() {
  const features = [
    { icon: <Brain className="w-6 h-6 text-primary" />, title: "Unlimited AI Companion", desc: "Your personal guide with complete context of every past session." },
    ...
    { icon: <FileText className="w-6 h-6 text-lavender" />, title: "Premium Life Reports", desc: "Generate beautifully crafted PDF summaries of your emotional growth." },
    ...
    <div className="mb-24 relative">
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {features.map((feature, i) => (
          <div key={i} className="bg-card/50 backdrop-blur-sm border border-white/10 rounded-3xl p-8 hover:bg-white/5 hover:border-white/20 transition-all group shadow-xl hover:shadow-2xl hover:shadow-primary/5">
            <div className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform shadow-inner">
              {feature.icon}
            </div>
