      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="relative w-full md:w-80 shrink-0">
          ...
          <input 
            ...
            className="w-full bg-surface-2 border border-line rounded-full py-2.5 pl-12 pr-4 text-sm text-white placeholder:text-[#9CA3AF]/70 focus:outline-none focus:border-brand/50 transition-colors"
          />
        </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto w-full pb-2 md:pb-0 scrollbar-hide">
          ...
          {['Specialization', 'Accuracy (90%+)', 'Price', 'Availability', 'Trust Score'].map(filter => (
            <button 
              key={filter}
              className="flex items-center gap-2 px-4 py-2.5 rounded-full bg-surface-2 border border-line text-sm font-medium text-white hover:bg-surface-3 hover:border-line-strong transition-colors shrink-0 whitespace-nowrap cursor-pointer"
            >
