...
export const SupportModal: React.FC<SupportModalProps> = ({ onClose }) => {
  ...
  const [subject, setSubject] = useState('Audio/Video Session Connection');
      ...
      <div className="relative w-full max-w-md bg-[#141518] border border-neutral-800 rounded-2xl p-6 shadow-2xl text-neutral-100">
        ...
        {ticketSent ? (
          <div className="py-10 text-center space-y-3">
            ...
            <button
              onClick={onClose}
              className="mt-4 px-4 py-2 bg-neutral-800 hover:bg-neutral-700 text-neutral-200 text-xs font-mono rounded-lg transition-colors cursor-pointer"
            >
              Return to Directory
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="mt-5 space-y-4 text-xs">
            ...
            <button
              type="submit"
              className="w-full py-2.5 bg-amber-500 hover:bg-amber-400 text-black font-mono font-medium rounded-xl transition-colors flex items-center justify-center gap-2 cursor-pointer"
            >
